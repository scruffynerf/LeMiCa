...
def get_Lemica_path(args, lemica_step=17, num_inference_steps=50):
    """
    Generate a boolean list based on args.ckpt_dir and lemica_step.
...
def lemica_forward(
    ...
    if self.enable_lemica:
        if not hasattr(self, "bool_list"):
            raise AssertionError("LeMiCa requires `self.bool_list` when thresholding is disabled.")
        if self.cnt >= len(self.bool_list):
            raise AssertionError(
                f"bool_list length ({len(self.bool_list)}) is shorter than current step ({self.cnt})."
            )

        should_calc = bool(self.bool_list[self.cnt])
        self.is_even = (self.cnt % 2 == 0)
        should_calc_even = should_calc
        should_calc_odd = should_calc
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Enable LEMICA if specified; disabled if omitted."
    )
...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        args.enable_lemica = args.lemica_budget is not None

        if args.enable_lemica:
            print(f"LEMICA enabled (budget = {args.lemica_budget})")
            # LeMiCa
            wan_t2v.__class__.generate = t2v_generate
            wan_t2v.model.__class__.enable_lemica = True
            wan_t2v.model.__class__.forward = lemica_forward
            ...
            # lemica
            bool_path = get_Lemica_path(args, lemica_step=args.lemica_budget, num_inference_steps=50)
            wan_t2v.model.__class__.bool_list= bool_path            
            
        else:
            print("LEMICA disabled")
        ...
        logging.info(f"WanT2V pipeline created successfully. Time taken: {elapsed:.2f} seconds.")  
        ...
        if args.enable_lemica:
            ...
            wan_i2v.model.__class__.enable_lemica = True
            wan_i2v.model.__class__.forward = lemica_forward
            ...
            # lemica
            bool_list = get_Lemica_path(args, lemica_step=args.lemica_budget, num_inference_steps=50)
            wan_i2v.model.__class__.bool_list= bool_list      
        ...
        end_time = time.time()
        elapsed = end_time - start_time
        logging.info(f"WanI2V pipeline created successfully. Time taken: {elapsed:.2f} seconds.")  
    ...
    if rank == 0:
        if args.save_file is None:
            ...
            args.save_file = (
                f"{args.task}_{args.size}_{args.ulysses_size}_{args.ring_size}_"
                f"{args.lemica_budget}_{formatted_prompt}_{formatted_time}_elapsed{elapsed:.2f}"
                + suffix
            )
